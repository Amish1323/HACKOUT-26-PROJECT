...
# ==========================================
st.set_page_config(
    page_title="AeroSolar Sentinel | AI Predictive Maintenance",
    page_icon="AeroSolar",
    layout="wide",
...
def call_gemini_api(api_key: str, prompt_text: str) -> str:
    ...
    if not clean_key:
        return "API Error: Key is empty or formatted incorrectly."

    # Method 1: Try official google.generativeai SDK
    try:
        import google.generativeai as genai
        genai.configure(api_key=clean_key)
        for model_id in ['gemini-1.5-flash', 'gemini-1.5-flash-latest', 'gemini-2.0-flash', 'gemini-1.5-pro']:
            try:
    ...
    # Method 2: Multi-endpoint REST fallback
    endpoints = [
    ...
    return f"API Call Error ({last_error}). Please verify your Gemini API key in Google AI Studio (https://aistudio.google.com)."

# ==========================================
# SIDEBAR CONTROLS
# ==========================================
st.sidebar.title("AeroSolar Sentinel")
...
st.sidebar.markdown("---")
use_ai_api = st.sidebar.checkbox("Enable Gemini AI Diagnostics", value=False, key="sb_use_ai_api")
api_key = ""
if use_ai_api:
    secret_key = st.secrets.get("GEMINI_API_KEY", "") if hasattr(st, "secrets") else ""
    api_key = st.sidebar.text_input("Gemini API Key", value=secret_key, type="password", key="sb_api_key")
    st.sidebar.caption("Press ENTER after entering your API key.")
...
<div class="top-header-banner">
    <div class="main-header">AeroSolar Sentinel: Fleet Monitoring & Intelligence</div>
...
# ==========================================
with st.expander("Revenue Loss Calculation Methodology & Financial Exposure", expanded=False):
    st.markdown("#### Monetized Loss Quantification & Physics Derivatives")
    st.write("AeroSolar Sentinel quantifies operational derating into daily monetary losses using real-time physics derating equations and commercial tariff constants.")
    
    calc_col1, calc_col2 = st.columns(2)
    with calc_col1:
        with st.container(border=True, height=385):
            st.markdown("##### Solar PV Loss Model")
            st.latex(r"	ext{Loss}_{	ext{Solar (INR)}} = (P_{	ext{Expected}} - P_{	ext{Actual}}) 	imes H_{	ext{Sun}} 	imes R_{	ext{Tariff}}")
            st.markdown(r"""
            **Variable Breakdown:**
            - **Expected Power ({	ext{Expected}}$)**: Capacity (kW) $	imes$ (Irradiance / 1000)
            - **Actual Power ({	ext{Actual}}$)**: {	ext{Expected}} 	imes (1 - 	ext{Total Loss \%}/100)$
            - **Total Derating (%)**: Thermal STC Derating (hiBHc0.4\%/	ext{°C} > 25	ext{°C}$) + Soiling Derating
            - **Operating Basis ({	ext{Sun}}$)**: 5 Peak Sun Hours / Day @ ₹4.50 / kWh
            - **Fleet Exposure**: Real-time monetized derating loss across active solar arrays
            """)
            st.info(f"**Current Fleet Solar Financial Exposure**: **₹{int(solar_rev_loss):,} / day**")
        
    with calc_col2:
        with st.container(border=True, height=385):
            st.markdown("##### Wind Turbine Loss Model")
            st.latex(r"	ext{Loss}_{	ext{Wind (INR)}} = 	ext{Capacity}_{	ext{kW}} 	imes D_{	ext{Derate}} 	imes H_{	ext{Ops}} 	imes R_{	ext{Tariff}}")
            st.markdown(r"""
            **Variable Breakdown:**
            - **Base Capacity ($	ext{Capacity}_{	ext{kW}}$)**: Nameplate Rated Output per Turbine in kW
            - **Derate Factor ({	ext{Derate}}$)**: -bash\%$ (Normal), 0\%$ (Warning), 0\%$ (Critical)
            - **Smart Weather Suppressor**: Drops {	ext{Derate}}$ to -bash\%$ when wind speed $\ge 18	ext{ m/s}$
            - **Operating Basis ({	ext{Ops}}$)**: 12 Operational Hours / Day @ ₹4.50 / kWh
            - **Fleet Exposure**: Real-time monetized derating loss across active wind turbines
            """)
            st.info(f"**Current Fleet Wind Financial Exposure**: **₹{int(wind_rev_loss):,} / day**")
